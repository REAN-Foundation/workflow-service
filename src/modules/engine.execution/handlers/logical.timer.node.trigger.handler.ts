...
export default class LogicalTimerNodeTriggerHandler {

    private static _q = asyncLib.queue((model: TimerNodeTriggerModel, onCompleted) => {
        (async () => {
            await LogicalTimerNodeTriggerHandler.process(model);
            onCompleted(model);
        })();
    }, ASYNC_TASK_COUNT);

    private static enqueue = (model: TimerNodeTriggerModel) => {
        LogicalTimerNodeTriggerHandler._q.push(model, (x, error) => {
            if (error) {
    ...
    static handle = async (model: TimerNodeTriggerModel) => {
        return new Promise((resolve, reject) => {
            try {
                LogicalTimerNodeTriggerHandler.enqueue(model);
                resolve(true);
            }
            catch (error) {
                reject(error);
